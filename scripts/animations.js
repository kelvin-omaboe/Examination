(function () {
  ...
  function randomPointInRing(rng, innerRadius, outerRadius) {
    ...
    var radiusSq = innerRadius * innerRadius + rng() * (outerRadius * outerRadius - innerRadius * innerRadius);
  ...
  AFRAME.registerComponent("float-bob", {
    ...
    tick: function (time) {
      ...
      var offset = Math.sin(time * 0.001 * this.data.speed + this.data.phase) * this.data.amplitude;
  ...
  AFRAME.registerComponent("stair-flight", {
    ...
    build: function () {
      ...
      for (var i = 0; i < steps; i++) {
        ...
        step.setAttribute(
          "geometry",
          "primitive: box; width: " + width.toFixed(3) + "; height: " + rise.toFixed(3) + "; depth: " + run.toFixed(3)
        );
        step.setAttribute(
          "position",
          "0 " + ((i + 0.5) * rise).toFixed(3) + " " + ((i + 0.5) * run).toFixed(3)
        );
        step.setAttribute(
          "material",
          "color: " + this.data.color + "; roughness: " + this.data.roughness + "; metalness: " + this.data.metalness
        );
      ...
      landing.setAttribute(
        "geometry",
        "primitive: box; width: " + width.toFixed(3) + "; height: 0.12; depth: " + this.data.landingDepth.toFixed(3)
      );
      landing.setAttribute(
        "position",
        "0 " + (steps * rise + 0.06).toFixed(3) + " " + (steps * run + this.data.landingDepth * 0.5).toFixed(3)
      );
      landing.setAttribute(
        "material",
        "color: " + this.data.color + "; roughness: " + this.data.roughness + "; metalness: " + this.data.metalness
  ...
  AFRAME.registerComponent("island-vegetation", {
    ...
    build: function () {
      ...
      var leafPalette = ["#3a7a3a", "#2f6d35", "#4a8a48", "#2f6742", "#568d51"];
      ...
      for (var i = 0; i < this.data.treeCount; i++) {
        var point = randomPointInRing(rng, this.data.innerClearRadius + 3, this.data.islandRadius - 1.6);
        ...
        if (noVegetationRects.some(function (rect) { return isPointInsideRect(point, rect); })) {
          continue;
        }

        var tree = document.createElement("a-entity");
        tree.setAttribute("position", point.x.toFixed(2) + " 0 " + point.z.toFixed(2));
        tree.setAttribute("rotation", "0 " + Math.floor(rng() * 360) + " 0");

        var trunkHeight = 2.4 + rng() * 1.5;
        var trunkRadius = 0.14 + rng() * 0.09;

        var trunk = document.createElement("a-entity");
        trunk.setAttribute("geometry", "primitive: cylinder; radius: " + trunkRadius.toFixed(3) + "; height: " + trunkHeight.toFixed(2) + "; segmentsRadial: 8");
        trunk.setAttribute("material", "color: #715230; roughness: 1; metalness: 0");
        trunk.setAttribute("position", "0 " + (trunkHeight * 0.5).toFixed(2) + " 0");
        trunk.setAttribute("shadow", "cast: true; receive: true");
        tree.appendChild(trunk);

        var canopy = document.createElement("a-entity");
        canopy.setAttribute("geometry", "primitive: sphere; radius: " + (0.95 + rng() * 0.42).toFixed(2) + "; segmentsWidth: 7; segmentsHeight: 6");
        canopy.setAttribute("material", "color: " + randomItem(rng, leafPalette) + "; roughness: 1");
        canopy.setAttribute("position", "0 " + (trunkHeight + 0.4).toFixed(2) + " 0");
        canopy.setAttribute("shadow", "cast: true; receive: true");
        tree.appendChild(canopy);

        var canopy2 = document.createElement("a-entity");
        canopy2.setAttribute("geometry", "primitive: sphere; radius: " + (0.75 + rng() * 0.32).toFixed(2) + "; segmentsWidth: 7; segmentsHeight: 6");
        canopy2.setAttribute("material", "color: " + randomItem(rng, leafPalette) + "; roughness: 1");
        canopy2.setAttribute("position", (rng() * 0.7 - 0.35).toFixed(2) + " " + (trunkHeight + 0.98).toFixed(2) + " " + (rng() * 0.7 - 0.35).toFixed(2));
        canopy2.setAttribute("shadow", "cast: true; receive: true");
      ...
      for (var j = 0; j < this.data.plantCount; j++) {
        var plantPoint = randomPointInRing(rng, this.data.innerClearRadius + 1.5, this.data.islandRadius - 0.7);
        if (plantPoint.x < -11 && plantPoint.z < -5.5) {
          continue;
        }
        if (noVegetationRects.some(function (rect) { return isPointInsideRect(plantPoint, rect); })) {
          continue;
        }
        ...
        if (j % 7 === 0) {
          ...
          flower.setAttribute("position", plantPoint.x.toFixed(2) + " 0 " + plantPoint.z.toFixed(2));
          flower.setAttribute("rotation", "0 " + Math.floor(rng() * 360) + " 0");
          var flowerScale = 0.55 + rng() * 0.4;
          flower.setAttribute("scale", flowerScale.toFixed(2) + " " + flowerScale.toFixed(2) + " " + flowerScale.toFixed(2));
          fragment.appendChild(flower);
          continue;
        }

        var shrub = document.createElement("a-entity");
        shrub.setAttribute("position", plantPoint.x.toFixed(2) + " 0 " + plantPoint.z.toFixed(2));
        shrub.setAttribute("rotation", "0 " + Math.floor(rng() * 360) + " 0");

        var leafA = document.createElement("a-entity");
        leafA.setAttribute("geometry", "primitive: cone; radiusBottom: " + (0.18 + rng() * 0.1).toFixed(2) + "; radiusTop: 0.02; height: " + (0.5 + rng() * 0.3).toFixed(2) + "; segmentsRadial: 6");
        leafA.setAttribute("material", "color: " + randomItem(rng, plantPalette) + "; roughness: 1");
        leafA.setAttribute("position", "0 " + (0.25 + rng() * 0.08).toFixed(2) + " 0");
        leafA.setAttribute("shadow", "cast: true; receive: true");
        shrub.appendChild(leafA);

        var leafB = document.createElement("a-entity");
        leafB.setAttribute("geometry", "primitive: cone; radiusBottom: " + (0.14 + rng() * 0.08).toFixed(2) + "; radiusTop: 0.01; height: " + (0.42 + rng() * 0.24).toFixed(2) + "; segmentsRadial: 6");
        leafB.setAttribute("material", "color: " + randomItem(rng, plantPalette) + "; roughness: 1");
        leafB.setAttribute("position", (rng() * 0.16 - 0.08).toFixed(2) + " " + (0.2 + rng() * 0.08).toFixed(2) + " " + (rng() * 0.16 - 0.08).toFixed(2));
        ...
        if (j % 9 === 0) {
          ...
          rock.setAttribute("geometry", "primitive: dodecahedron; radius: " + rockSize.toFixed(2));
          rock.setAttribute("material", "color: #7f7f7f; roughness: 1");
          rock.setAttribute("position", (rng() * 0.28 - 0.14).toFixed(2) + " " + (rockSize * 0.6).toFixed(2) + " " + (rng() * 0.28 - 0.14).toFixed(2));
  ...
  AFRAME.registerComponent("day-night-cycle", {
    ...
    applyMode: function (mode) {
      ...
      this.el.setAttribute(
        "fog",
        "type: exponential; color: " + (isDay ? "#9ed3f7" : "#111f35") + "; density: " + (isDay ? "0.0038" : "0.0055")
      );
  ...
  AFRAME.registerComponent("toggle-day-night", {
    ...
    refreshLabel: function () {
      ...
      var target = this.data.target || this.el.sceneEl;
      var mode = target && target.components["day-night-cycle"] ? target.components["day-night-cycle"].mode : "day";
      this.data.label.setAttribute("value", mode === "day" ? "Mode: Day" : "Mode: Night");
    }
  });
  ...
  AFRAME.registerComponent("toggle-light-group", {
    ...
    init: function () {
      ...
      this.lightEls = this.data.targets ? Array.prototype.slice.call(scene.querySelectorAll(this.data.targets)) : [];
      this.indicatorEls = this.data.indicators ? Array.prototype.slice.call(scene.querySelectorAll(this.data.indicators)) : [];
      this.defaultIntensity = new Map();
      this.isOn = true;

      for (var i = 0; i < this.lightEls.length; i++) {
        var lightComp = this.lightEls[i].getAttribute("light");
        this.defaultIntensity.set(this.lightEls[i], lightComp && typeof lightComp.intensity === "number" ? lightComp.intensity : 1);
      }
    ...
    refreshVisuals: function () {
      for (var i = 0; i < this.indicatorEls.length; i++) {
        this.indicatorEls[i].setAttribute(
          "material",
          "color: #ffd187; emissive: " + (this.isOn ? "#f5b460" : "#1f1f1f") + "; emissiveIntensity: " + (this.isOn ? 0.85 : 0.08) + "; roughness: 0.4"
        );
      }

      if (this.data.label) {
        this.data.label.setAttribute("value", this.isOn ? "Dock: On" : "Dock: Off");
      }
    }
  });
